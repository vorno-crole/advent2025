...
# functions
	...
	postProcessA()
	{
		# load output, remove lines that do not require more processing...
		# echo " - load array"
		local output_file=$1
		local output_file2=$2

		# echo "postProcessA $output_file"

		declare -A outputLines
		local line sum value lineUpValue lineDnValue
		while read -u 11 -r line sum; do
			# echo $line,$sum
			outputLines[$line]=$sum
		done 11< <(awk -F'|' '{print $1" "$3}' ${output_file})

		# echo " - scan array"
		local deleteLines=""
		for line in "${!outputLines[@]}"; do
			# echo "Key: $line"
			# echo "Value: ${outputLines[$line]}"

			value="${outputLines[$line]}"
			...
			# echo $line:$value
			# echo -ne "Key: $line Value: $value"
			# echo " [!! 0]"

			lineUpValue="${outputLines[$((line-1))]}"
			lineDnValue="${outputLines[$((line+1))]}"
			# echo $lineUpValue:$lineDnValue
			...
			if (( value + lineUpValue + lineDnValue == 0 )); then
				# echo "up and down all zeroes"
				deleteLines+="${line}d;"
			# else
			# 	echo "non-zero. do not delete"
			fi
		done
		...
		# echo " - drop lines"
		if [[ $deleteLines != "" ]]; then
			sed "$deleteLines" "${output_file}" > ${output_file2}
		else
...
# postProcessA output3.txt output3.txt2
# exit;
...
if (( sum > 0 )); then
	...
	postProcessA "${output_file}" "${output_file}2"

	
	awk -F'|' '{print $2}' ${output_file}2 | tr '0123' '...' | tr '456789' '@@@@@@' > input$nextRun.txt

	$0 input$nextRun.txt $nextRun 
	exit;
else

	echo -e "cannot optimise any further.\n"
